...
impl AsyncTradeContext {
    ...
    fn us_order_detail(&self, py: Python<'_>, order_id: String, is_attached: bool) -> PyResult<Py<PyAny>> {
        let ctx = self.ctx.clone();
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
    ...
    fn us_realized_pl(&self, py: Python<'_>, currency: String, category: Option<String>) -> PyResult<Py<PyAny>> {
        let ctx = self.ctx.clone();
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let r: crate::trade::types::USRealizedPL =
                ctx.us_realized_pl(currency, category).await.map_err(ErrorNewType)?.into();
            Ok(r)
        })
        .map(|b| b.unbind())
    }
