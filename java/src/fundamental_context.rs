...
pub unsafe extern "system" fn Java_com_longbridge_SdkNative_fundamentalContextFinancialReport(
    ...
    jni_result(&mut env, (), |env| {
        ...
        let kind_opt: Option<JavaInteger> = get_field(env, &opts, "kind")?;
        let kind = match kind_opt.map(i32::from).unwrap_or(3) {
            0 => FinancialReportKind::IncomeStatement,
            1 => FinancialReportKind::BalanceSheet,
            2 => FinancialReportKind::CashFlow,
            _ => FinancialReportKind::All,
        };
        let period_opt: Option<JavaInteger> = get_field(env, &opts, "period")?;
        let period = period_opt.map(i32::from).and_then(|v| match v {
