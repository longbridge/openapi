...
- **All languages:** add `AgentContext.chats` (`GET /v1/ai/chats`) and `AgentContext.chat` (`GET /v1/ai/chats/{chat_uid}`) — list the account's chats (conversations) across Agents (optional `page` / `limit` / `exclude_agent_uids`), and fetch a single chat's detail with its messages. New types: `Chat`, `ChatsResponse`, `ChatMessage`, `ChatMessageChunk`, `ChatInfo`, `ChatDetail`, `GetChatsOptions`. The deeply-nested `chat_relation` and message `extends` are surfaced as raw JSON (JSON string in C/C++/Java). Available in the Rust, Python, Node.js, Java, and C/C++ bindings
...
- **Java SDK:** `AgentContext.publicAgents` is now `synchronized`, like every other native-handle-backed instance method. It was added after the sweep that made these methods mutually exclusive with `close()` and missed the modifier, leaving the same `close()`-races-a-live-call use-after-free window this method was supposed to be protected against
