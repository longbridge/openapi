...
- **C:** `lb_fund_context_*` FFI (all 28 methods via `c/src/fund_context`) with the `C*Options` request structs and all 39 fund response structs (`lb_fund_detail_t`, `lb_hot_fund_t`, `lb_fund_order_t`, …) emitted into `c/csrc/include/longbridge.h` via the cbindgen `include` list.
