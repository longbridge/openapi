...
- **Rust core (async + blocking):** new `FundContext` / `FundContextSync` for the mutual-fund channel, covering 28 endpoints across fund catalog & market data (hot funds, fund list, filters, detail, analysis/trend, annual/quarterly returns, performance & comparison, latest/historical NAV, top-10 holdings, reverse stock holdings), the user's fund positions (overview, single position, performance, profits, NAV history, dividends), and fund orders/trading (order list & detail, transactions/cash-flow, order validate/submit/cancel). Identifiers are exposed as `symbol` (the OpenAPI gateway maps them to the backend `counter_id`). Mirrored across the C, C++, Java, Node.js and Python layers; the separate Go SDK is pending.
- **C:** `lb_fund_context_*` FFI (all 28 methods via `c/src/fund_context`) with `C*Options` request structs, regenerated into `c/csrc/include/longbridge.h`. Note: the fund response structs are currently reached through the `void*` async-result data pointer and are **not yet emitted into the public header** (unlike grid's) — a follow-up is needed to add them to the cbindgen `include` list.
...
- **C / C++:** resolved a cbindgen name collision introduced with the fund C FFI layer: the fund module's `CFundPosition` and `CGetFundPositionsOptions` shared their names with the existing portfolio types, so the regenerated `longbridge.h` overwrote `lb_fund_position_t` and `lb_get_fund_positions_options_t` with the fund field layouts, breaking the C++ trade/portfolio converters. The fund structs were renamed to `CFundPositionItem` (reached only through the `void*` async data pointer, so not emitted into the header) and `CFundPositionsOptions` (`lb_fund_positions_options_t`, a `lb_fund_context_positions` parameter) so both modules coexist.
...
- **All SDKs:** `FundamentalContext.valuation_comparison` now sends its peer list as a `comparison_symbols` query array (repeated-key HTTP array) instead of the legacy `comparison_counter_ids`, completing the symbol↔counter_id removal for this endpoint. The client-side `symbol_to_counter_id` stopgap has been removed; the public API is unchanged (it has always taken user symbols). Verified live against production (#581)
