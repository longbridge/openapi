...
impl FundamentalContext {
    ...
    pub async fn us_company_overview(&self, counter_id: String) -> Result<USCompanyOverview> {
        Ok(self.ctx.us_company_overview(counter_id).await.map_err(ErrorNewType)?.into())
    }
    ...
    pub async fn us_valuation_overview(&self, counter_id: String) -> Result<USValuationOverview> {
        Ok(self.ctx.us_valuation_overview(counter_id).await.map_err(ErrorNewType)?.into())
    }

    /// Get US financial overview (JSON string). US token required.
    #[napi]
    pub async fn us_financial_overview(&self, counter_id: String, report: String) -> Result<String> {
        let v = self.ctx.us_financial_overview(counter_id, report).await.map_err(ErrorNewType)?;
    ...
    #[napi]
    pub async fn us_financial_statement_v3(&self, counter_id: String, kind: String, report: String) -> Result<USFinancialStatement> {
        Ok(self.ctx.us_financial_statement_v3(counter_id, kind, report).await.map_err(ErrorNewType)?.into())
    }

    /// Get US key financial metrics (JSON string). US token required.
    #[napi]
    pub async fn us_key_financial_metrics(&self, counter_id: String, report: String) -> Result<String> {
        let v = self.ctx.us_key_financial_metrics(counter_id, report).await.map_err(ErrorNewType)?;
        serde_json::to_string(&v).map_err(|e| napi::Error::from_reason(e.to_string()))
    }
    ...
    pub async fn us_analyst_consensus(&self, counter_id: String, report: String) -> Result<String> {
        let v = self.ctx.us_analyst_consensus(counter_id, report).await.map_err(ErrorNewType)?;
        serde_json::to_string(&v).map_err(|e| napi::Error::from_reason(e.to_string()))
    }
    ...
    pub async fn us_etf_dividend_info(&self, counter_id: String) -> Result<USETFDividendInfo> {
        Ok(self.ctx.us_etf_dividend_info(counter_id).await.map_err(ErrorNewType)?.into())
    }
    ...
    pub async fn us_company_dividends(&self, counter_id: String) -> Result<USCompanyDividends> {
        Ok(self.ctx.us_company_dividends(counter_id).await.map_err(ErrorNewType)?.into())
    }

    /// Get US ETF document list. size=None returns all. US token required.
    #[napi]
    pub async fn us_etf_files(&self, counter_id: String, size: Option<u32>) -> Result<USETFFilesResponse> {
        Ok(self.ctx.us_etf_files(counter_id, size).await.map_err(ErrorNewType)?.into())
    }
}
