...
use crate::{
    decimal::Decimal,
    grid::types::{GridLimitEvent, GridTimeInForce, TriggerPriceType},
};
...
impl<'env> From<GridTradeRule<'env>> for longbridge::grid::GridTradeRule {
    fn from(r: GridTradeRule<'env>) -> Self {
        longbridge::grid::GridTradeRule {
            ...
            trigger_price_type: r.trigger_price_type.map(Into::into),
            ...
            time_in_force: r.time_in_force.map(Into::into),
            ...
            upper_limit_event: r.upper_limit_event.map(Into::into),
            lower_limit_event: r.lower_limit_event.map(Into::into),
