...
inline market::MarketTimeItem convert(const lb_market_time_item_t* item) {
  return { item->market, item->trade_status, item->timestamp, item->delay_trade_status, item->delay_timestamp, item->sub_status, item->delay_sub_status };
}
...
inline fundamental::InstitutionRating convert(const lb_institution_rating_t* r) {
  ...
  fundamental::InstitutionRatingSummary summary { r->summary.ccy_symbol, r->summary.change, convert(r->summary.evaluate), r->summary.recommend, r->summary.target, r->summary.updated_at };
...
inline portfolio::ProfitSummaryInfo convert(const lb_profit_summary_info_t* item) {
  return { item->asset_type, item->profit_max, item->profit_max_name, item->loss_max, item->loss_max_name };
}
...
inline portfolio::ProfitAnalysisItem convert(const lb_profit_analysis_item_t* item) {
  return { item->name, item->market, item->is_holding, item->profit, item->profit_rate,
           item->clearance_times, item->item_type, item->currency, item->symbol,
...
inline portfolio::FlowItem convert(const lb_flow_item_t* item) {
  return { item->executed_date, item->executed_timestamp, item->code, item->direction,
...
inline dca::DcaPlan convert(const lb_dca_plan_t* p) {
  return { p->plan_id, p->status, p->symbol, p->member_id, p->aaid, p->account_channel,
           p->display_account, p->market, p->per_invest_amount, p->invest_frequency,
